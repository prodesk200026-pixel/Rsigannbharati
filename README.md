...
## v3 update — broker files flattened (no more `brokers/` subfolder)

The `brokers/` subfolder kept disappearing when uploading to GitHub from
mobile (drag-drop / mobile upload flows don't reliably preserve nested
folders), causing the exact same `Cannot find module './brokers/dhan'`
crash a second time — even though the zip itself was correct both times.
To remove that failure mode entirely: `dhan.js` and `angelone.js` now
live at the **repo root**, next to `server.js`, `config.js`, etc. — this
zip has no subfolders at all. Just upload every file straight into your
repo's root and you're done; there's no folder structure to lose.
